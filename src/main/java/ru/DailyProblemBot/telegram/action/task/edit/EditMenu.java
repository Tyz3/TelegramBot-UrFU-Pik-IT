...
public class EditMenu extends Action {
    ...
    public EditMenu(UserPhase phase) {
        super(phase);
    }
    ...
    private static void showTasks(String chatId, UserProperties properties, boolean moveLeftToRight) {
        ...
        int count = Main.getSettings().SHOWEDIT_TASKS_AT_ONCE;

        List<Task> tasks = TaskService.findAFewForEdit(count, properties.getLastShowTaskId(), moveLeftToRight);

        // Если заявок на голосование совсем нет
        if (tasks.size() == 0) {

            if (moveLeftToRight) {
                tasks = TaskService.findFirstForEdit(count);
            } else {
                tasks = TaskService.findEndForEdit(count);
            }
    ...
    private static void sendTask(String chatId, UserProperties properties, List<Task> tasks) {
        ...
        if (tasks.size() == 0) {
            Messages.tasks_edit_nothingToShow.send(chatId);
            ActionEngine.getAction(UserPhase.MAIN).alert(chatId, properties);
            return;
        }
        ...
        Messages.tasks_edit_intro.send(chatId, Keyboard.TASK_EDIT);
